...
*/
uint8_t packet_received = RADIO_FLAG_IDLE;

/*
 * 表示传输是否完成的标志。
 * 初始值为RADIO_FLAG_IDLE，表示传输未完成。
*/
uint8_t packet_transmit = RADIO_FLAG_IDLE;
...
RF_Err_t rf_init(void)
{
    ...
    RF_ASSERT(rf_write_reg(REG_OP_MODE, RF_MODE_DEEP_SLEEP));//0x02,0 深度睡眠模式
    rf_port.delayus(10);
		
    RF_ASSERT(rf_write_reg(REG_OP_MODE, RF_MODE_SLEEP));//睡眠模式
    rf_port.delayus(10);
		
    RF_ASSERT(rf_set_spec_page_reg_bits(PAGE3_SEL, 0x06, BIT5));//切换页面，读出值，切换页面写值|更改值
    rf_port.delayus(10);

    RF_ASSERT(rf_write_reg(REG_OP_MODE, RF_MODE_STB1));//STB1模式
    rf_port.delayus(10);
    ...
    RF_ASSERT(rf_write_reg(REG_OP_MODE, RF_MODE_STB2));//STB2模式
    rf_port.delayus(150);

    RF_ASSERT(rf_write_reg(REG_OP_MODE, RF_MODE_STB3));//STB3模式
    rf_port.delayus(10);
...
RF_Err_t rf_set_default_para(uint32_t freq)
{
    ...
    RF_ASSERT(rf_set_crc(CRC_ON));//设置有效载荷CRC
		RF_ASSERT(rf_set_tx_power(16));//DEFAULT_PWR));//设置低功耗发射功率最小1 最大23
    return OK;
}
